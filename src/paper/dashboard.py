...
<footer>
  Generated by <a href="https://github.com/gaaiyun/kalshi-ai-trading-bot">kalshi-ai-trading-bot</a> · Paper trading mode · Not financial advice
</footer>
...
if (data.length > 0) {{
  new Chart(document.getElementById('pnlChart'), {{
    ...
    if output_path:
        ...
        # The HTML contains non-ASCII characters (emoji, ellipsis, "¢"). Force
        # UTF-8 so generation does not crash on platforms whose default text
        # encoding is not UTF-8 (e.g. GBK on Windows).
...
def generate_dashboard(output_path: str = None) -> str:
    """
    Generate the paper-trading dashboard and write it to disk.

    Defaults to the documented location ``docs/paper_dashboard.html`` relative
    to the project root. Returns the path that was written.
    """
    if output_path is None:
        output_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "docs", "paper_dashboard.html"
        )
